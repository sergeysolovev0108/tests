...
var s3 = new AWS.SecretsManager({
    region: region
});

function getAwsSecret(secretName){
  return s3.getSecretValue({ SecretId: secretName }).promise();
}
...
async function main(argv, setIo) {
  ...
  if (argv.connect) {
    ...
    box = await get_local_box(argv.key,argv.secretName, argv.cert, argv.ca_path);
  }
